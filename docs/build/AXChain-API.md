...
curl -X POST --data '{
    ...
    "id": 1
}' -H 'content-type:application/json;' https://1.p2p-v2.testnet.axiacoin.network:443/ext/bc/AX/rpc
```
...
To interact with other instances of the EVM AXC endpoints:

```
/ext/bc/blockchainID/axc
```
...
curl -X POST --data '{
    ...
    "params" :{
        "txID":"KsyVbDe1yQJtdrDkJ6aSXn82hZss68xSeHZZw33E9CjYBEpTs7bMD45bjpp95Bogn2k9dYUag71VXs6mQ9CRjMjN4s",
        "encoding": "hex"
    }
}' -H 'content-type:application/json;' https://1.p2p-v2.testnet.axiacoin.network:443/ext/bc/AX/axc
```
...
Finalize the transfer of a non-AXC or AXC from Swap-Chain or Core-Chain to the AX Chain. Before this method is called, you must call the Swap-Chain’s [`avm.export`](x-chain.md#avmexport) or Core-Chain’s [`platform.exportAXC`](p-chain.md#platformexportaxc) with assetID `AXC` to initiate the transfer.
...
- `sourceChain` is the ID or alias of the chain the asset is being imported from. To import funds from the Swap-Chain, use `"X"`; for the Core-Chain, use `"P"`.
...
Finalize a transfer of AXC from the Swap-Chain or Core-Chain to the AX Chain. Before this method is called, you must call the Swap-Chain’s [`avm.export`](x-chain.md#avmexport) or Core-Chain’s [`platform.exportAXC`](p-chain.md#platformexportaxc) with assetID `AXC` to initiate the transfer.
...
- `sourceChain` is the ID or alias of the chain the AXC is being imported from. To import funds from the Swap-Chain, use `"X"`; for the Core-Chain, use `"P"`.
- `baseFee` is the base fee that should be used when creating the transaction. If omitted, a suggested fee will be used.
- `username` is the user that controls the address that transaction will be sent from.
- `password` is `username`‘s password.
...
}' -H 'content-type:application/json;' https://1.p2p-v2.testnet.axiacoin.network:443/ext/bc/AX/admin
```
