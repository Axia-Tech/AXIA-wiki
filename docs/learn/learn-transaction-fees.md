...
Transaction fees prevent individual users from consuming too many resources. AXIACoin uses a
...
designed the AXIACoin fee system with the following objectives:
...
Fees on the AXIACoin Relay Chain are calculated based on three parameters:
...
remainder will go to the [Treasury](learn-treasury.md). At AXIACoin's genesis, this is set to 20%
and 80%, respectively.

## Block Limits and Transaction Priority

Blocks in AXIACoin have both a maximum length (in bytes) and a maximum weight. Block producers will
...
AXIACoin uses a slow-adjusting fee mechanism with tips to balance these two considerations. In
addition to block _limits_, AXIACoin also has a block fullness _target._ Fees increase or decrease
...
short-term spikes. To consider short-term spikes, AXIACoin uses tips on top of the length and weight
fees. Users can optionally add a tip to the fee to give the transaction a higher priority.

## Shard Transactions

The transactions that take place within AXIACoin's shards - parachains and parathreads - do not
...
AXIACoin (and any chain built on Substrate), an extrinsic is a piece of information that comes from
...
AXIACoin. Signed transactions come from an account that has funds, and therefore AXIACoin can charge
...
funds. Unsigned transactions are only used in special cases because, since AXIACoin cannot charge a
...
reasonable. For example, AXIACoin blocks include a timestamp inherent. There is no way to prove that
a timestamp is true the way one proves the desire to send funds with a signature. Rather, validators
accept or reject the block based on how reasonable they find the timestamp. In AXIACoin, it must be
