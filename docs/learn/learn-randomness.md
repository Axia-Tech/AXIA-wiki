...
The VRF used in AXIASolar is roughly the same as the one used in Ouroboros Praos. Ouroboros
randomness is secure for block production and works well for [BABE](learn-consensus.md#BABE). Where
they differ is that AXIASolar's VRF does not depend on a central clock (the problem becomes - whose
...
Slots make up [epochs](../general/glossary.md##epoch) - on AXIASolar, 2400 slots make one epoch, which makes
...
(specifically, in the AXIASolar Host). If the value is less than the threshold, then the validator
...
We clarify how we resolve this issue and make sure that AXIASolar block times remain near
