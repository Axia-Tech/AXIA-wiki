...
You might want to host a node on one server and then connect to it from a UI hosted on another, e.g. [AXIASolarJS UI](https://axiasolar.js.org/apps). This will not be possible unless you set up a secure proxy for websocket connections. Let's see how we can set up WSS on a remote Substrate node.
...
Open [AXIASolarJS UI](https://axiasolar.js.org/apps) and click the logo in the top left to switch node. Activate the "Custom Endpoint" toggle and input your node's address - either the domain or the IP address. Remember to prefix with `wss://`!

![A sync-in-progress chain connected to AXIASolar UI](/img/wss/wss01.jpg)
