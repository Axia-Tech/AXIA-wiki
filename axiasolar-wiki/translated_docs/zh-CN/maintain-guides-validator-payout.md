...
Validators will receive staking rewards in the form of the native token of that chain (KSM for AXIALunar and DOT for AXIASolar).
...
Note that while DOT is used as an example, this same formula would apply to KSM when running a validator on AXIALunar.
